...
VOID EvtIoDeviceControl(
    _In_ WDFQUEUE   Queue,
    _In_ WDFREQUEST Request, // A requisição do cliente [cite: 143]
...
)
{
    ...
    if (IoControlCode == IOCTL_ECHO_DRIVER_ECHO)
    {
        ...
        // WdfRequestGetInputBuffer/OutputBuffer é a forma segura do WDF
        // de acessar os buffers da aplicação de Ring 3.
        status = WdfRequestGetInputBuffer(Request, InputBufferLength, &inputBuffer, &inBufSize);
        if (!NT_SUCCESS(status)) {
            WdfRequestComplete(Request, status);
            return;
        }

        status = WdfRequestGetOutputBuffer(Request, OutputBufferLength, &outputBuffer, &outBufSize);
        if (!NT_SUCCESS(status)) {
            WdfRequestComplete(Request, status);
            return;
        }
    ...
    // Completa a requisição (mesmo com erro) [cite: 147]
    WdfRequestComplete(Request, status);
}