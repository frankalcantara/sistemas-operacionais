    ...
    void monitor_function() {
        ...
        while (!processing_complete_.load()) {
            ...
            if (localtime_s(&tm_struct, &time_t_var) != 0) {
                std::cerr << "Erro ao converter tempo local. Usando valor padr�o.\n";
                // Opcional: Continue ou use UTC
                continue;
            }
            auto tm = tm_struct;  // Use como antes
            // Obt�m �ltimos primos encontrados
            auto last_primes = stats_.get_last_primes();
            // Limpa o terminal inteiro e move o cursor para o topo
            std::cout << "\033[2J\033[H";
            // Exibe estat�sticas em um bloco coeso, com timestamp apenas no topo
            std::cout << "[" << std::put_time(&tm, "%H:%M:%S") << "] Estat�sticas de Processamento:\n"
                ...
                << "Taxa: " << std::fixed << std::setprecision(0) << rate << " n�meros/segundo\n"
                << "Tempo m�dio: " << std::fixed << std::setprecision(1) << avg_time << "ms por n�mero\n";
            // Exibe �ltimos primos
            if (!last_primes.empty()) {
                std::cout << "�ltimos primos encontrados: ";
