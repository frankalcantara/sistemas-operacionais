# Agendamento de Tarefas e Escalonamento

O escalonamento **F**irst-**C**ome, **F**irst-**S**erved (**FCFS**) demonstra o compromisso fundamental entre simplicidade e otimização de performance. Embora **FCFS** proporcione justiça e elimine starvation, **o efeito comboio cria degradação severa de performance quando processos longos precedem processos curtos**. 

O escalonamento **S**hortest **J**ob **F**irst (**SJF**) alcança tempos médios de espera ótimos entre algoritmos não-preemptivos**, mas requer conhecimento antecipado dos tempos de _burst_ de `CPU` que sistemas reais não podem prever de forma confiável. A variante preemptiva (Shortest Remaining Time First) melhora a responsividade, mas introduz overhead de mudança de contexto. Pesquisas universitárias demonstram a otimalidade teórica do **SJF** enquanto destacam limitações práticas por meio da estimação de tempo de _burst_ usando algoritmos de média exponencial.
