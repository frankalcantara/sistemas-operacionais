...
git commit -m "feat: adiciona funcionalidade X para resolver issue #123"
```
...
git commit -m "fix: corrige problema apontado na revisão"
...
git commit -m "feat: descrição consolidada das alterações (#PR-number)"
```
...
```bash
# Como mantenedor
gh pr merge PR-NUMBER --merge    # merge commit
...
git add .
git commit -m "fix: adiciona melhorias solicitadas"
git push origin feature/minha-contribuicao
```
...
```bash
# Criar nova branch com o commit
